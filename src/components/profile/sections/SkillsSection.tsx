    ...
    <div id="skills" className="bg-card rounded-xl border border-border p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
          ...
          {t("candidate.profile.sections.skills.title", t("candidate.profile.skills", "Key Skills"))}
        </h2>
      ...
      {profile?.skills && profile.skills.length > 0 ? (
        ...
        <p className="text-muted-foreground text-sm">
          {t(
            "candidate.profile.sections.skills.addSkillsToProfile",
            t("candidate.profile.addSkillsPrompt", "Add skills to showcase your expertise")
          )}
        </p>
      )}
      ...
      <Dialog open={isEditing} onOpenChange={setIsEditing}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {t(
                "candidate.profile.sections.skills.addSkill",
                t("candidate.profile.editKeySkills", "Edit Key Skills")
              )}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="relative">
              <Input
                placeholder={t(
                  "candidate.profile.sections.skills.enterSkill",
                  t("candidate.profile.typeSkillPlaceholder", "Enter a skill")
                )}
                value={skillInput}
                onChange={(e) => {
            ...
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">
                {t("candidate.profile.sections.skills.suggestions", t("candidate.profile.suggestions", "Suggestions"))}:
              </p>
              <div className="flex flex-wrap gap-2">
                {(skillInput
                  ? filteredSuggestions
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSave} disabled={updateProfile.isPending}>
              {updateProfile.isPending ? t("candidate.profile.saving", "Saving...") : t("common.save")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
